...
class Agent:
    ...
    def __init__(self, environment, min_trading_unit=1, max_trading_unit=2, delayed_reward_threshold=.05):
        ...
        self.base_portfolio_value = 0  # 직전 학습의 pv
    ...
    def get_states(self):
        self.ratio_hold = self.num_stocks / int(self.portfolio_value / self.environment.get_pirce())
    ...
    def validate_action(self, action):
        ...
        if action == Agent.ACTION_BUY:
            # 적어도 1주를 살수 있는지 확인
            if self.balance < self.environment.get_pirce() * (1 + self.TRADING_CHARGE) * self.min_trading_unit:
                validity = False
    ...
    def act(self, action, confidence):
        ...
        # 환경에서 현재 가격 얻기
        current_price = self.environment.get_pirce()
