...
# 일단 a2c의 형태로 만듦, 추후 a3c포맷으로 수정할것임
# class Trader(ReinforcementLearner):
#     def __init__(self, *args, **kwargs, value_network_path=None, policy_network_path=None ):
#         super().__init__(*args, **kwargs)
#         self.value_network_path = value_network_path
#         self.policy_network_path = policy_network_path
...
#      def run(
#         self, num_epoches=1, balance=10000000,
#         discount_factor=0.9, start_epsilon=0, learning=True):
#         info = "[{code}] RL:{rl} Net:{net} LR:{lr} " \
#             "DF:{discount_factor} TU:[{min_trading_unit}," \
#             "{max_trading_unit}] DRT:{delayed_reward_threshold}".format(
#             code=self.stock_code, rl=self.rl_method, net=self.net,
...
#                     kw.send_order("send_order_req", "0101", account, 1, self.stock_code, max(min(int(self.agent.balance / (self.environment.get_price * (1 + self.agent.TRADING_CHARGE))),self.agent.max_trading_unit),self.agent.min_trading_unit
#                 ), self.environment.get_price(), "00", "")
#                 elif action == self.agent.ACTION_SELL:
#                     kw.send_order("send_order_req", "0101", account, 2, self.stock_code, max(min(int(self.agent.balance / (self.environment.get_price * (1 + self.agent.TRADING_CHARGE))),self.agent.max_trading_unit),self.agent.min_trading_unit
#                 ), self.environment.get_price(), "00", "")
